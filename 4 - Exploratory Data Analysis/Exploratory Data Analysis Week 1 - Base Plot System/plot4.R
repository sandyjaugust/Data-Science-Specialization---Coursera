...
#Build grid for plots
par(mfrow = c(2,2), mar = c(4,4,1,1))

#Create all plots
#Top left quadrant
with(df, plot(Time, Global_active_power, type = "n", ylab = "Global Active Power", xlab = NA))
with(df, lines(Time, Global_active_power))

#Top right quadrant
with(df, plot(Time, Voltage, type = "n", xlab = "datetime"))
...
with(df, legend("topright", legend = c("Sub_metering_1","Sub_metering_2","Sub_metering_3"), col = c("black","red","blue"), lty = 1, bty = "n", cex = .75))
